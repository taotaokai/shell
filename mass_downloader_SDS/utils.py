...
def download_mseed_bulk(client, client_name, chunks, logger):
    # Only the filename is not needed for the actual data request.
    bulk = [list(_i[0:6]) for _i in chunks]
...
def download_and_split_mseed_bulk(client, client_name, chunks, archive_root, logger):
    """
    ...
    bulk = [list(_i[0:6]) for _i in chunks] # KTAO: explicity take the first 6 values
    ...
    def _get_dbfile(archive_root, net, sta, loc, chan, starttime):
        yjd = starttime.strftime("%Y.%j")
        year = yjd[:4]
        return os.path.join(archive_root, year, net, sta, f"{chan}.D",
                               f"{net}.{sta}.{loc}.{chan}.{yjd}")
        ...
        with open(temp_filename, "rb") as fh:
            try:
                while True:
                    ...
                    filename = _get_dbfile(archive_root, info["network"], info["station"],
                                  info["location"], info["channel"], info['starttime'])
                    # Again sometimes there are time ranges nobody asked for...
                    if filename is None:
                        fh.read(info["record_length"])
                        continue
                    #>> KTAO: changed to write out file simultaneously
                    # if filename not in open_files:
                    #     open_files[filename] = open(filename, "wb")
                    # open_files[filename].write(fh.read(info["record_length"]))
                    if filename not in open_files:
                        open_files[filename] = b''
                    open_files[filename] += fh.read(info["record_length"])
            finally:
                #>> KTAO: merge with existing mseed file
                for filename in open_files:
                    os.makedirs(os.path.dirname(filename), exist_ok=True)
                    rec = open_files[filename]
                    st_existed = None
                    if os.path.exists(filename):
                        try:
                            st_existed = obspy.read(filename, format='MSEED')
                        except Exception as e:
                            logger.warning(f'failed to read {filename} ({e})')
                    if st_existed:
                        with io.BytesIO(rec) as f:
                            st_new = obspy.read(f, format='MSEED')
                        st = st_existed + st_new
                        st.merge(-1)
                        logger.debug(f'existed: {st_existed}')
                        logger.debug(f'new: {st_new}')
                        logger.debug(f'merged: {st}')
