...
twin_polar = [-30.0, -20, 40, 50]
twin_decon = [-160.0, -150, 60, 70.0]

cmpnm_list = ['.BHE', '.BHN', '.BHZ']

# main phase name
main_phase_names =  ['S', 'SKS']
...
for sacFile in sacFile_list:
  ...
  if w[1] > w[0]:
    u0 = np.dot(v[:,1]*np.sign(v[0,1]), RZ) # main phase
    u1 = np.dot(v[:,0]*np.sign(v[1,0])*(-1), RZ) # converted phase
  else:
    u0 = np.dot(v[:,0]*np.sign(v[0,0]), RZ) # main phase
    u1 = np.dot(v[:,1]*np.sign(v[1,1])*(-1), RZ) # converted phase 

  #u0 = RZ[1,:] 
  #u1 = RZ[0,:] 

  #------ deconvolution
  Fu0 = np.fft.rfft(u0[::-1])
  Fu1 = np.fft.rfft(u1[::-1])
...
ax1.set_title("%s: S-RF"%(stnm))
ax1.set_ylabel("Ray parameter (s/deg)")

outfig = "%s_SRF_a%.2f.pdf"%(stnm, tau)
